/**
 * WordPress → Payload CMS: Trailheads Import Script
 *
 * Usage:
 *   npx tsx scripts/import-trailheads.ts
 *
 * Prerequisites:
 ...
 *
 * Dry run:
 *   DRY_RUN=true npx tsx scripts/import-trailheads.ts
 *
 * Notes:
 *   - Elevation is NOT sent to Payload; the elevationBeforeChangeHook on the
 *     Trailheads collection fetches it automatically from lat/lng.
 *   - Re-running is safe: existing trailheads (matched by _wp_id) are skipped.
 *   - Requires the updated Trailheads.ts that includes _wp_id and _wp_slug fields.
 */
...
// ---------------------------------------------------------------------------
// Step 1: Fetch all trailheads from WordPress
// ---------------------------------------------------------------------------

async function fetchAllWpTrailheads(): Promise<WpTrailhead[]> {
  const all: WpTrailhead[] = []
  let page = 1

  console.log('📥  Fetching trailheads from WordPress…')

  while (true) {
    const url = `${WP_BASE_URL}/wp-json/wp/v2/trailhead?per_page=${WP_PER_PAGE}&page=${page}&_fields=id,title,content,slug,status,acf`
    ...
    const batch: WpTrailhead[] = await res.json()
    if (batch.length === 0) break

    all.push(...batch)
    console.log(`  Page ${page}: fetched ${batch.length} trailheads (total so far: ${all.length})`)

    if (batch.length < WP_PER_PAGE) break
    page++
  }

  console.log(`✅  Total WordPress trailheads fetched: ${all.length}\n`)
  return all
}
...
// ---------------------------------------------------------------------------
// Step 3: Check which trailheads already exist (by _wp_id) to avoid dupes
// ---------------------------------------------------------------------------
...
async function fetchExistingWpIds(token: string): Promise<Set<number>> {
  ...
  while (true) {
    const res = await fetch(`${PAYLOAD_URL}/api/trailheads?limit=100&page=${page}&depth=0`, {
...
function mapTrailhead(wp: WpTrailhead): PayloadTrailhead {
  const raw = wp.content?.rendered ?? ''
  const description = stripHtml(raw)

  return {
    trailhead: wp.title.rendered
      .replace(/&amp;/g, '&')
      .replace(/&#8211;/g, '–')
      .replace(/&#8212;/g, '—')
      .replace(/&#038;/g, '&')
...
// ---------------------------------------------------------------------------
// Step 5: POST each trailhead to Payload
// ---------------------------------------------------------------------------

async function createPayloadTrailhead(token: string, payload: PayloadTrailhead): Promise<void> {
  const res = await fetch(`${PAYLOAD_URL}/api/trailheads`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (!res.ok) {
    const text = await res.text()
    throw new Error(`Failed to create "${payload.trailhead}" (${res.status}): ${text}`)
  }
}
...
async function main() {
  ...
  const wpTrailheads = await fetchAllWpTrailheads()
  ...
  if (!DRY_RUN) {
    ...
    console.log('🔍  Checking for already-imported trailheads…')
  ...
  const toImport = wpTrailheads.filter((t) => !existingIds.has(t.id))

  if (DRY_RUN) {
    console.log(`🔍  DRY RUN — nothing will be written to Payload\n`)
  }

  console.log(`📤  ${DRY_RUN ? 'Would import' : 'Importing'} ${toImport.length} new trailheads…\n`)

  let success = 0
  const failures: Array<{ name: string; wpId: number; reason: string }> = []

  for (const wp of toImport) {
    const mapped = mapTrailhead(wp)

    if (DRY_RUN) {
      console.log(`  🔍  [DRY RUN] ${mapped.trailhead} (WP id: ${wp.id})`)
    ...
    try {
      await createPayloadTrailhead(token, mapped)
      console.log(`  ✅  ${mapped.trailhead} (WP id: ${wp.id})`)
      success++
    } catch (err) {
      console.log(`  ❌  ${mapped.trailhead} (WP id: ${wp.id}) — see failures summary below`)
      failures.push({ name: mapped.trailhead, wpId: wp.id, reason: (err as Error).message })
    }
  }
...
`)
  ...
  if (wpTrailheads.some((t) => !t.acf?.latitude || !t.acf?.longitude)) {
    const missing = wpTrailheads.filter((t) => !t.acf?.latitude || !t.acf?.longitude)
    console.log(`⚠️  NOTE: ${missing.length} trailhead(s) are missing lat/lng in WordPress:`)
